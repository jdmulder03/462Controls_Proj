...
HAL_StatusTypeDef MPU6050_Init(I2C_HandleTypeDef* hi2c)
{
    ...
    MPU6050_ConfigTypeDef cfg = {.DLPF_CFG = 0x00};
...
HAL_StatusTypeDef MPU6050_ReadAll(I2C_HandleTypeDef* hi2c, MPU6050_RawDataTypeDef* out)
{
    ...
    out->accel.x = (int16_t)__builtin_bswap16(out->accel.x);
    out->accel.y = (int16_t)__builtin_bswap16(out->accel.y);
    out->accel.z = (int16_t)__builtin_bswap16(out->accel.z);
    out->temp = (int16_t)__builtin_bswap16(out->temp);
    out->gyro.x = (int16_t)__builtin_bswap16(out->gyro.x);
    out->gyro.y = (int16_t)__builtin_bswap16(out->gyro.y);
    out->gyro.z = (int16_t)__builtin_bswap16(out->gyro.z);
